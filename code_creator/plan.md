...
### Concrete build steps
1. Accept ticket JSON (from Step 1) + schema JSON (from Step 2) as file args; optionally accept `--sql-content <file>` containing a Claude-generated SQL body
...
5. Assemble SQL file:
   - `{{ config(...) }}` block with all 8 required meta keys (`originator`/`maintainer` = assignee; others empty string if unknown)
   - If `--sql-content` provided: append its contents as the model body (Claude has already written the CTE + SELECT logic)
   - If `--sql-content` not provided (CI/fallback only): generate a scaffold body from schema columns with a TODO comment — this is a fallback, not the primary path
6. Generate schema.yml addition:
   - Model entry with description
   - Columns = the **model's output columns** (what the new model SELECTs), NOT the source table columns
   - If `--output-columns <file>` provided: use that JSON list `[{"name": "...", "description": "..."}]` — this is the authoritative list written by Claude after generating the SQL body
   - If `--output-columns` not provided (CI/fallback): derive columns from source schema JSON as a best-effort fallback, flagged with a TODO comment
   - `unique` + `not_null` on surrogate key or primary grain column; `dbt_utils.unique_combination_of_columns` for composite keys
7. Via GitHub API:
   a. Get `dev` branch HEAD SHA
   b. Create branch `feature/CNGBA-<id>`
   c. Commit SQL file
   d. Commit schema.yml update (append new model entry to existing file)
8. Print branch name and file paths to stdout

### Acceptance criteria
| Scenario | Expected behaviour |
|----------|--------------------|
| `--sql-content` provided | SQL file = config block + Claude-generated body; no scaffold fallback used |
| `--output-columns` provided | schema.yml lists model output columns (e.g. `month_year`, `total_requests`) — never source table columns |
| `--sql-content` not provided (CI path) | SQL file = config block + scaffold with explicit TODO comment; acceptable for CI, not for interactive use |
| `--output-columns` not provided (CI path) | schema.yml derived from source schema JSON with TODO markers |
...
4. **Generate SQL body** — Claude reads the ticket description and schema, then writes the dbt SQL body to a temp file:
   - Extract business logic: explicit SQL snippets in the ticket, named metrics, filter conditions, aggregations, GROUP BY
   - Use `{{ ref('model') }}` for internal dbt models, `{{ source('system', 'table') }}` for raw sources (never hardcode catalog paths)
   - Must include at least one transformation beyond a column rename (aggregation, filter, derived column, or ratio)
   - Columns must be explicit — no `SELECT *`
   - Do NOT include the `{{ config() }}` block — `create_branch.py` adds that
   - If the ticket's business logic is ambiguous, ask the user one clarifying question before generating
5. Run `python scripts/create_branch.py --ticket <file> --schema <file> --sql-content <file>` — capture branch name and file paths
6. Post Jira comment: `"Branch \`feature/CNGBA-<id>\` created. Draft model at \`<sql path>\`. Open a PR manually when ready."`
7. Print summary to Claude Code chat

### Acceptance criteria
| Scenario | Expected behaviour |
|----------|--------------------|
| Full run on valid ticket | Chat shows: domain inferred, SQL logic summary, files created, branch name, Jira comment posted |
| Generated SQL is a flat column dump with no transformations | Must not happen — Claude must re-read ticket and generate real logic |
...
| Ticket description has explicit SQL snippet | Claude adapts that snippet to dbt style (refs, sources, no hardcoded paths) as the model body |
